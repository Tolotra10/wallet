...
import { PendingQRTransaction, Transaction, Wallet,QRCodeModel } from "../models/index.js"
...
router.post("/generate-qr", authenticateAdmin, async (req, res) => {
  try {
    ...
    // Générer l'image QR (base64)
    const qrCodeData = await QRCode.toDataURL(JSON.stringify(qrPayload));

    // Enregistrer dans la table qr_codes
    await QRCodeModel.create({
      user_id: userId || null,
      admin_id: req.admin.id,
      pending_qr_transaction_id: pendingTransaction.id,
      qr_data: JSON.stringify(qrPayload),  // <- utiliser qr_data
      qr_image: qrCodeData,                // <- image base64
      category,
      amount,
      expires_at,
      is_active: true,
